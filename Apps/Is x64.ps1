...
begin {
  ...
  function WriteErrorBadEXE {
    $MyErrorRecord = [ErrorRecord]::new(
      [IO.FileFormatException]::new("The following is not a valid Windows EXE: ""$LiteralPath"""),
      'BadFileFormat',
...
process {
  Write-Verbose -Message "Testing ""$LiteralPath"""
  if (-not ([IO.File]::Exists($LiteralPath))) {
      $MyErrorRecord = [ErrorRecord]::new(
        [IO.FileNotFoundException]::new("The specified path does not lead to a file: ""$LiteralPath""", $LiteralPath),
        'ObjectNotFound',
